...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        if deep_stem:
            channels_stem = int(32 * stem_factor)
            self.conv1 = nn.Conv2d(3, channels_stem, kernel_size=3, stride=2,
                                   padding=1, bias=False)
            self.bn1 = nn.BatchNorm2d(channels_stem)
            self.relu = nn.ReLU(inplace=True)
            self.conv2 = nn.Conv2d(channels_stem, channels_stem*2,
                                   kernel_size=3, stride=1,
                                   padding=1, bias=False)
            self.bn2 = nn.BatchNorm2d(channels_stem*2)
            self.conv3 = nn.Conv2d(channels_stem*2, channels_stem*2,
                                   kernel_size=3, stride=2,
                                   padding=1, bias=False)
            self.bn3 = nn.BatchNorm2d(channels_stem*2)
        else:
            channels_stem = int(64 * stem_factor)
            if kernel_size_stem == 7:
                self.conv1 = nn.Conv2d(3, channels_stem, kernel_size=7, stride=2,
                ...
                self.bn1 = nn.BatchNorm2d(channels_stem)
                self.relu = nn.ReLU(inplace=True)
                self.conv2 = nn.Conv2d(channels_stem, channels_stem,
                                       kernel_size=3, stride=2,
                                       padding=1, bias=False)
                self.bn2 = nn.BatchNorm2d(channels_stem)
            else:
    ...
    def make_layer(block_class,
        ...
        for i in range(num_blocks):
            if 'se_list' in kwargs:
                kwargs['with_se'] = kwargs['se_list'][i]
            layers.append(
                block_class(
...
def get_cls_model(config, **kwargs):
    ...
    stem_factor = 1 if 'STEM_FACTOR' not in spec else spec.STEM_FACTOR
    in_channels = 64 * stem_factor
    ...
    for idx, num_block in enumerate(num_blocks):
        ...
        if isinstance(with_se, list):
            kwargs['se_list'] = with_se[idx]
        else:
    ...
    if config.FINETUNE.FINETUNE:
        model.freeze_pretrained_layers(config.FINETUNE.FROZEN_LAYERS)
