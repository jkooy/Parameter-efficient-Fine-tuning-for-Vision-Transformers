...
def matvec_product(W: torch.Tensor, x: torch.Tensor, bias: torch.Tensor,
    ...
    # print(f'phm_rule {phm_rule.size()}')
    # print(f'W {W.size()}')
    # print(f'H {H.size()}, {x.size()}')
...
class HyperComplexAdapter(nn.Module):
    ...
    def __init__(
        ...
        # # self.down_PHM = PHMLinear(in_features=self.input_dim,
        ...
        # seq_list.append(nn.Linear(self.input_dim, self.down_sample_size))
        # # seq_list.append(self.down_PHM)
        
        # self.down_sampler = nn.Sequential(*seq_list)
...
class MultiheadAttention(nn.Module):
    ...
    def multi_head_attention_forward(
        ...
        # q_delta = self.kadapter(query).reshape(B*self.num_heads, N, C // self.num_heads)
        # v_delta = self.kadapter(key).reshape(B*self.num_heads, N, C // self.num_heads)
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, kattention = None):
        ...
        if kattention is not None:
            ...
            # *[ResidualAttentionBlock(width, heads, attn_mask, kattention) for _ in range(1)],
