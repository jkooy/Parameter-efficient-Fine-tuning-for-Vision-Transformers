...
class CLIP_wrapper(nn.Module):
  ...
  def __init__(
    ...
    self.adapter_model = models.cls_vit_lora.get_cls_model(config)
...
def classifier(train_dataloader, val_dataloader, test_dataloader, no_hyperparameter_tuning, lr, l2, config, args):
    ...
    logging.info("=> The lora layer and classifier is on training ...")
...
def get_dataloader(dataset, val_split=0, batch_size_per_gpu=8, workers=6, pin_memory=True):
    if val_split == 0:
...
def linear_probe():
    ...
    final_output_dir = create_logger(config, args.cfg, 'finetune_lora')
