...
class PHMLinear(torch.nn.Module):
    def __init__(self,
        ...
        # self.H = nn.Parameter(torch.Tensor(size=(in_features, out_features)),
        #                       requires_grad=True)
    ...
    def forward(self, x: torch.Tensor, phm_rule: Union[None, nn.ParameterList] = None) -> torch.Tensor:
        ...
        # (4,4,4) (4,16,192)
        # H: (64,768)
        # x: (50,50,64) (50,50,768)
        #   self.H.size())
        # y = F.linear(x, self.H.t(), self.b)

        # y = torch.matmul(input=x, other=self.H)
        # y += self.b
        # return y
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_compacter is not None:
            ...
            print(f"=> Compacter forward.")
        else:
            x = x + self.mlp(self.ln_2(x))
        # try:
        #     x = x + self.compacter(self.mlp(self.ln_2(x)))
        #     print(f"=> Compacter forward.")
        #     # x = x + self.compacter(self.mlp(self.ln_2(x)), self.mlp(self.ln_2(x)))[0]
        # except:
        #     x = x + self.mlp(self.ln_2(x))
        return x
