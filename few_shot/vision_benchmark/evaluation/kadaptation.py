...
def matvec_product(W: torch.Tensor, x: torch.Tensor, bias: torch.Tensor,
                       phm_rule,
...
class Attention(nn.Module):
    def __init__(self,
        ...
        # # self.phm_rank = 4
        # self.phm_rank = 1
        # self._in_feats_per_axis = 1
        # # self._in_feats_per_axis = 2
        # self._out_feats_per_axis = 1
        # # self._out_feats_per_axis = 2
        # out_features = phm_dim * self._out_feats_per_axis
        self.phm_init_range = 0.01
        self.phm_rule = nn.Parameter(torch.FloatTensor(phm_dim, phm_dim, phm_dim), 
                       requires_grad=True)

        self.W_left1 = nn.Parameter(torch.Tensor(size=(phm_dim, self._in_feats_per_axis, self.phm_rank)),
                              requires_grad=True)
        self.W_right1 = nn.Parameter(torch.Tensor(size=(phm_dim, self.phm_rank, self._out_feats_per_axis)),
                              requires_grad=True)
        self.W_left2 = nn.Parameter(torch.Tensor(size=(phm_dim, self._in_feats_per_axis, self.phm_rank)),
                              requires_grad=True)
        self.W_right2 = nn.Parameter(torch.Tensor(size=(phm_dim, self.phm_rank, self._out_feats_per_axis)),
                              requires_grad=True)
        # self.W_left3 = nn.Parameter(torch.Tensor(size=(phm_dim, self._in_feats_per_axis, self.phm_rank)),
        #                       requires_grad=True)
        # self.W_right3 = nn.Parameter(torch.Tensor(size=(phm_dim, self.phm_rank, self._out_feats_per_axis)),
        #                       requires_grad=True) 
        # self.W_left4 = nn.Parameter(torch.Tensor(size=(phm_dim, self._in_feats_per_axis, self.phm_rank)),
        #                       requires_grad=True)
        # self.W_right4 = nn.Parameter(torch.Tensor(size=(phm_dim, self.phm_rank, self._out_feats_per_axis)),
        #                       requires_grad=True)             
        self.phmb = nn.Parameter(torch.Tensor(out_features))                                  
        for i in range(phm_dim):
            self.W_left1.data[i].normal_(mean=0, std=self.phm_init_range)
            self.W_right1.data[i].normal_(mean=0, std=self.phm_init_range)
            self.W_left2.data[i].normal_(mean=0, std=self.phm_init_range)
            self.W_right2.data[i].normal_(mean=0, std=self.phm_init_range)
            # self.W_left3.data[i].normal_(mean=0, std=self.phm_init_range)
            # self.W_right3.data[i].normal_(mean=0, std=self.phm_init_range)
            # self.W_left4.data[i].normal_(mean=0, std=self.phm_init_range)
            # self.W_right4.data[i].normal_(mean=0, std=self.phm_init_range)


    def phm_forward(self, x, phm_rule, w):
    ...
    def forward(self, x, prev=None):
        hidden_states = x
        # logging.info(f'=> hidden_states size is {hidden_states.size()}')
        B, N, C = x.shape
        qkv = self.qkv(x) \
        ...
        W1 = torch.bmm(self.W_left1, self.W_right1)
        W2 = torch.bmm(self.W_left2, self.W_right2)
        # W3 = torch.bmm(self.W_left3, self.W_right3)
        # W4 = torch.bmm(self.W_left4, self.W_right4)
        q_delta = self.phm_forward(lora_input, self.phm_rule, W1).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v_delta = self.phm_forward(lora_input, self.phm_rule, W2).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        # logging.info(f'=> input size is {x.size()}')
        B = x.shape[0]
...
def get_cls_model(config, **kwargs):
    ...
    if config.MODEL.NAME in clip.available_models(): 
        vit = VisionTransformer(
            ...
            patch_size=vit_spec.VISION.PATCH_SIZE,
            num_classes=config.MODEL.NUM_CLASSES,
            embed_dim=vit_spec.VISION.EMBED_DIM,
            qkv_bias=vit_spec.VISION.QKV_BIAS,
            depth=vit_spec.VISION.DEPTH,
            num_heads=vit_spec.VISION.NUM_HEADS,
            mlp_ratio=vit_spec.VISION.MLP_RATIO,
            drop_rate=vit_spec.VISION.DROP_RATE,
            attn_drop_rate=vit_spec.VISION.ATTN_DROP_RATE,
            drop_path_rate=vit_spec.VISION.DROP_PATH_RATE,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),
            use_cls_tocken=vit_spec.VISION.USE_CLS_TOKEN,
...
class Classifier(torch.nn.Module):
    ...
    def __init__(self, config, l2_lambda):
        ...
        for n, param in self.backbone.named_parameters():
            # if 'W' in n or 'phmb' in n:
            #     logging.info(f'{n}, shape: {param.shape}, require grad.')
            if 'W' in n:
                logging.info(f'{n}, shape: {param.shape}, require grad.')
            else:
                logging.info(f"=> Paramters in the model not require grad: {n}, shape: {param.shape}.")
...
def kadapt(train_dataloader, val_dataloader, test_dataloader, no_hyperparameter_tuning, lr, l2, config):
    # best_lr = lr
    # best_l2_lambda = l2
    ...
    performance_metrics.update({"Total time in minutes ": total_time})
    performance_metrics.update({"Memory utilization in GB": peak_memory})
    logging.info(f"{performance_metrics}")
