...
class Attention(nn.Module):
    ...
    def get_lepe(self, x, func):
        ...
        lepe = func(x) ### B', C, H', W'
        lepe = lepe.reshape(-1, self.num_heads, C // self.num_heads, H * W).permute(0, 1, 3, 2).contiguous()
        return lepe

    def forward(self, x, prev=None):
        B, N, C = x.shape
        qkvo = self.qkv(x).reshape(B, N, 3, C).permute(2, 0, 1, 3)
        qkv = qkvo.reshape(B, N, 3, self.num_heads, C // self.num_heads) \
                  .permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)

        attn_score = (q @ k.transpose(-2, -1)) * self.scale

        lepe = self.get_lepe(qkvo[2], self.get_v)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        use_cls_tocken = False   ## match LePE size
        if use_cls_tocken:
...
class Classifier(torch.nn.Module):
    ...
    def __init__(self, config, l2_lambda):
        ...
        if os.path.exists("vit.pth"):
            ...
            backbone = torch.load('vit.pth')
        rpb_model_dict = self.backbone.state_dict()
        state_dict = {k:v for k,v in backbone.items() if k in rpb_model_dict.keys() and "pos_embed" not in k}
        rpb_model_dict.update(state_dict)
        self.backbone.load_state_dict(rpb_model_dict)
...
def lepe(train_dataloader, val_dataloader, test_dataloader, no_hyperparameter_tuning, lr, l2, config):
