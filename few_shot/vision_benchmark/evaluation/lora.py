...
class Classifier(torch.nn.Module):
    ...
    def __init__(self, config, l2_lambda):
        ...
        if os.path.exists("vit.pth"):
            ...
            backbone = torch.load('vit.pth')
        lora_model_dict = self.backbone.state_dict()
        state_dict = {k:v for k,v in backbone.items() if k in lora_model_dict.keys()}
        lora_model_dict.update(state_dict)
        self.backbone.load_state_dict(lora_model_dict)
...
def lora(train_dataloader, val_dataloader, test_dataloader, no_hyperparameter_tuning, lr, l2, config):
