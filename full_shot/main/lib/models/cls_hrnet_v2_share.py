...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
        ...
        self.num_branches_split = self._get_num_channels_split(num_inchannels)
    ...
    def _get_num_channels_split(self, num_channels):
        num_channels_split = [[num_channels[0]]]
        for i in range(1, len(num_channels)):
            num_channels_curr = copy.deepcopy(num_channels_split[-1])
            num_channels_curr.append(
                num_channels[i]-sum(num_channels_curr[:i])
            )
            num_channels_split.append(num_channels_curr)

        return num_channels_split
    ...
    def _make_one_branch(
        ...
        if branch_index != 0:
            num_channels_in -= self.num_inchannels[branch_index-1]
            num_channels_out -= num_channels[branch_index-1] * block.expansion
        ...
        layers = [
            block(
                ...
                num_channels[branch_index]-num_channels[branch_index-1]
                if branch_index != 0 else num_channels[branch_index],
    ...
    def forward(self, x):
        ...
        for i in range(1, self.num_branches):
            x_split = torch.split(x[i], self.num_branches_split[i], dim=1)
            x_split_out = []
            j = 0
            while j <= i:
                x_split_out.append(self.branches[j](x_split[j]))
                j += 1
            x[i] = torch.cat(x_split_out, dim=1)
