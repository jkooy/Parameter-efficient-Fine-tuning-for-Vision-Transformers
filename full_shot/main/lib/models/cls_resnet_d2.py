...
class BottleneckBlock(CNNBlockBase):
    ...
    def __init__(
        ...
        *,
        ...
        ) if avg_down and stride != 1 else None
        ...
        if in_channels != out_channels:
            self.shortcut = Conv2d(
                ...
                out_channels,
                kernel_size=1,
                stride=1 if avg_down else stride,
                bias=False,
                norm=get_norm(norm, out_channels),
            )
        else:
    ...
    def forward(self, x):
        ...
        if self.shortcut is not None:
            shortcut = self.shortcut(self.avg_pool(x) if self.avg_pool else x)
        else:
            shortcut = x
...
class BasicStem(CNNBlockBase):
    ...
    def __init__(self, in_channels=3, out_channels=64, norm="BN", kernel_size=7):
        """
        Args:
...
def get_cls_model(cfg, **kwargs):
    ...
    stem = BasicStem(
        ...
        out_channels=64,
        norm=norm,
        kernel_size=spec.KERNEL_SIZE_STEM
    )
    ...
    # It consumes spec memory and may cause allreduce to fail
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        if depth in [18, 34]:
            ...
            stage_kargs['with_se'] = with_se
        blocks = ResNet.make_stage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
        bottleneck_channels *= 2
        stages.append(blocks)
    return ResNet(stem, stages, num_classes=cfg.MODEL.NUM_CLASSES)
