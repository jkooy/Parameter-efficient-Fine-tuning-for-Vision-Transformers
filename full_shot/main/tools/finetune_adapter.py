...
from evaluation.dataset import construct_dataloader
...
class CLIP_wrapper(nn.Module):
  ...
  def __init__(
    ...
    self.adapter_model = models.cls_vit_adapter.get_cls_model(config)
    logging.info(f'new model: {self.adapter_model}.')
...
def train_one(train_loader, model, criterion, optimizer, epoch, config):
    ...
    try:
        for index,  batch in enumerate(train_loader):
            logging.info(f'Index of train_loader is', index)
            images, target = batch[:2]
...
def classifier(train_dataloader, val_dataloader, test_dataloader, no_hyperparameter_tuning, lr, l2, config, args):
    logging.info("=> The adapter layer and classifier is on training ...")
...
def linear_probe():
    ...
    final_output_dir = create_logger(config, args.cfg, 'finetune_adapter')
