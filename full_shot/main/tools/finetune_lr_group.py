...
from optim import build_optimizer_different_lr
...
def adjust_learning_rate_group(optimizer, epoch, config):
    """Decay the learning rate based on schedule"""
    lr = config.TRAIN.LR
    for milestone in config.TRAIN.SCHEDULE:
        lr *= 0.1 if epoch >= milestone else 1.
    
    print('parameter group', optimizer)
...
def train_task(train_dataloader, val_dataloader, config):
    ...
    optimizer = build_optimizer_different_lr(config, model)

    cudnn.benchmark = True
    

    for epoch in range(config.TRAIN.BEGIN_EPOCH, config.TRAIN.END_EPOCH):
        adjust_learning_rate_group(optimizer, epoch, config)
...
def train_one(train_loader, model, criterion, optimizer, epoch, config):
    ...
    for _, (images, target) in enumerate(train_loader):
        ...
        if epoch % 10 == 0:
            model_checkpoint = os.path.join(config.OUTPUT_DIR, f'{epoch}.pth')
            torch.save(model.state_dict(), model_checkpoint)
            for  n, p in model.named_parameters():
                if p.grad is not None:
                    logging.info(f'epoch {epoch}, parameter name {n}, norm {torch.norm(p)}, grad_norm {torch.norm(p.grad)}') 
...
def classifier(train_dataloader, val_dataloader, test_dataloader, no_hyperparameter_tuning, lr, l2, config, args):
    print('no_hyperparameter_tuning', no_hyperparameter_tuning)
...
def linear_probe():
    ...
    final_output_dir = create_logger(config, args.cfg, 'lr_group')
