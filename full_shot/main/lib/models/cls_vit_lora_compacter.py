...
def matvec_product(W: torch.Tensor, x: torch.Tensor,
    ...
    # logging.info(f'=> phm_rule size is {phm_rule.size()}')
    # logging.info(f'=> W size is {W.size()}')
    # logging.info(f'=> H size is {H.size()}')
    # logging.info(f'=> x size is {x.size()}')
...
class Attention(nn.Module):
    ...
    def forward(self, x, prev=None):
        ...
        # q_delta = self.adapter_forward(lora_input, self.q_proj_adapter1.weight, self.q_proj_adapter2.weight, g_weight=self.q_moe_adapter1).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        # logging.info(f'=> q_delta size is {q_delta.size()}')
        # logging.info(f'=> q size is {q.size()}')
